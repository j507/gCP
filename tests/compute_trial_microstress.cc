...
namespace Tests
{
  ...
  private:
    ...
    gCP::TrialMicrostress<dim> trial_microstress;
  ...
  template <int dim>
  void LinearAlgebra<dim>::check_dof_index()
  {
    {
             ...
             fe_field.get_dof_handler().active_cell_iterators())
        {
          if (cell->is_locally_owned())
          {
            ...
            for (unsigned int i = 0;
                 i < dof_indices.size(); ++i)
            {
              // if (fe_field.get_global_component(crystal_id, i) >= dim)
              {
                affine_constraints.add_line(dof_indices[i]);
    ...
    if (dealii::Utilities::MPI::sum(
          static_cast<int>(flag_condition), MPI_COMM_WORLD) ==
            n_processors)
    {
      *pcout << "Passed" << std::endl;
    }
    ...
    fe_field.reset_all_affine_constraints();

    trial_microstress.reset_all_affine_constraints();
  }
  ...
  void LinearAlgebra<dim>::check_projection()
  {
    ...
    const unsigned int n_quadrature_points =
        trial_microstress_hp_fe_values.get_quadrature_collection().max_n_quadrature_points();
         ...
         trial_microstress.get_dof_handler().active_cell_iterators())
    {
      if (cell->is_locally_owned())
      {
        ...
        {
          fe_field_fe_values[fe_field.get_slip_extractor(crystal_id, slip_id)].get_function_values(
              fe_field.solution,
              slip_values[slip_id]);
        }
        ...
        trial_microstress.get_hanging_node_constraints().distribute_local_to_global(
            local_matrix,
            local_dof_indices,
            matrix);
    ...
    { // Solve
...

      {
        for (unsigned int entry_id = 0;
             ...
             entry_id++)
        {
          if (trial_microstress.get_locally_owned_dofs().is_element(entry_id))
          {
  ...
  void LinearAlgebra<dim>::eliminate_active_set()
  {
    ...
    const unsigned int n_quadrature_points =
        fe_field_hp_fe_values.get_quadrature_collection().max_n_quadrature_points();
         ...
         fe_field.get_dof_handler().active_cell_iterators())
    {
      if (fe_field_cell->is_locally_owned())
      {
        ...
        fe_field.get_hanging_node_constraints().distribute_local_to_global(
            local_matrix,
